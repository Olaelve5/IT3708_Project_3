...
function representative_solution(front::Vector{Individual})::Individual
...
function front_stats(front::Vector{Individual})
	if isempty(front)
		return (size = 0, best_accuracy = NaN, min_features = 0, median_features = NaN)
	end
...
function print_generation_summary(label::AbstractString, gen::Int, front::Vector{Individual})
	s = front_stats(front)
	println(
